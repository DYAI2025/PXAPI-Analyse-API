...
| --- | --- | --- | --- | --- |
| C-PXAPI-001 | OPEN / NON-BLOCKING FOR PXAPI-4 | GitHub PR #5 remains open although current `main` has independently evolved far beyond its old base. | PR #5 base `b84a283...`, head `ed9814b...`; current `main@3a52929...` | Treat PR #5 as stale legacy delivery drift. It is not current WIP and must not be merged without a separate reconcile. No automatic close/delete. |
...
| C-PXAPI-004 | OPEN / SOURCE-BOUND | Confluence product amendment previously described the installed Skill baseline while GitHub `pxk-web-skill` now contains multiple uploaded ZIPs. | PXAPI amendment `52527106`; private repo `DYAI2025/pxk-web-skill@f29767f...` | PXAPI-4 must bind parity to the exact V2.1 artifact/digest documented in current Skill Confluence, not to an unqualified repo `latest`. A later Skill version requires a new parity rebaseline. |
